...
namespace mod_epsynthesis\form;
...
/**
 * Création / édition d'un EP académique partagé : intitulé, nombre d'ECTS qu'il porte, années
 * d'étude auxquelles il s'adresse et nombre de places.
 *
 * Le type d'EP ne se choisit pas ici, contrairement aux EP propres à une promotion : les types
 * sont définis dans chaque activité « Enseignement personnalisé », et un EP partagé est suivi par
 * des étudiants de plusieurs d'entre elles. Il relève donc du type académique interne de
 * l'instance de chaque étudiant, avec les plafonds de sa promotion à lui.
 *
 * @package   mod_epsynthesis
 * @copyright 2026 Sébastien Lefebvre
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class shared_activity_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('select', 'minstudyyear', get_string('minstudyyear', 'mod_ep'),
            ep_studyyear_options());
        $mform->setDefault('minstudyyear', 0);
        $mform->addHelpButton('minstudyyear', 'sharedstudyyearrange', 'mod_epsynthesis');

        $mform->addElement('select', 'maxstudyyear', get_string('maxstudyyear', 'mod_ep'),
            ep_studyyear_options());
